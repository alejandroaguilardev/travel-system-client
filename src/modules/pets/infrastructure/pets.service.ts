...
export const petService: PetService = {
    ...
    updateMeasurementsAndWeight: async (id: string, body: TopicoMeasurementsAndWeight): Promise<ResponseSuccess> => {
        const { data } = await axiosInstance.patch<ResponseSuccess>(`${endpoints.pets.root}/${id}/measurement`, body);
        return data;
    },
